...
def importer_image_manuelle():
    # Demande un chemin à l'utilisateur et copie l'image dans le dossier images/    
    if not os.path.exists(DOSSIER_IMAGES):
        os.makedirs(DOSSIER_IMAGES)

    chemin_source = input("Chemin de l'image (ou glisse le fichier ici) : ").strip()
    
    # Nettoyage du chemin (Windows ajoute souvent des guillemets " autour des chemins copiés)
    chemin_source = chemin_source.strip('"').strip("'")

    if not os.path.exists(chemin_source):
        return {"erreur": "Erreur : Le fichier n'existe pas ou le chemin est incorrect."}

    # Vérification simple de l'extension
    extensions_valides = ('.jpg', '.jpeg', '.png', '.webp')
    if not chemin_source.lower().endswith(extensions_valides):
        return {"erreur": "Erreur : Format non supporté. Utilise : {extensions_valides}"}

    # Copie du fichier
    nom_fichier = os.path.basename(chemin_source) # Récupère juste "photo.jpg"
    destination = os.path.join(DOSSIER_IMAGES, nom_fichier)

    try:
        shutil.copy2(chemin_source, destination) # copy2 garde les métadonnées
        return {"message": f"Image importée avec succès dans : {destination}"}
    except Exception as e:
        return {"erreur": f"Erreur lors de la copie : {e}"}